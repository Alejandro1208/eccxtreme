...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(EDITOR_URL + "popups/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
    window.open(EDITOR_URL + "popups/fullscreen.html?"+objname,
                'FullScreen',
  ...
  // Custom2
    ...
    var myText = showModalDialog(EDITOR_URL + "popups/custom2.html",
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(EDITOR_URL + "popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(EDITOR_URL + "popups/insert_image.html", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(EDITOR_URL + "popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +EDITOR_URL+ 'popups/blank.html"
...
function editor_about(objname) {
  showModalDialog(EDITOR_URL + "popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
